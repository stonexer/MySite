...
/**浏览器提示*/
function browser() {
	var browserHtml = '<div style="position:fixed;top:100px;right:-604px;z-index:10001;width:604px;height:373px;">\
			<img alt="兼容提示" src="./resources/500d/images/browser.png">\
			<a style="display:block;width:20px;height:20px;position:absolute;top:25px;right:22px;cursor:pointer;"></a>\
		</div>';
	if($.browser.msie) {
		if($.browser.version <= 8) {
			var browserDiv = $(browserHtml);
			browserDiv.click(function(){
				browserDiv.animate({right:-604}, 800);
			});
			$("body").append(browserDiv);
			browserDiv.animate({right:0}, 1000, function(){
				setTimeout(function(){
					browserDiv.animate({right:-604}, 800);
				}, 20 * 1000);
			});
